...
if source == "Upload video":
    ...
    if uploaded_file:
        ...
        upload_dir = Path(__file__).parent.parent / "database"
...
if uploaded_files:
    ...
    if st.button("Add Person", key="add_auth"):
        if name.strip() == "":
            ...
            # Define person's folder: database/<safe_name>/
            base_upload_dir = Path(__file__).parent.parent / "database"
            person_dir = base_upload_dir / safe_name
            person_dir.mkdir(parents=True, exist_ok=True)

            # Determine the next image number
            existing_images = list(person_dir.glob("*.*"))
            ...
            for uploaded_file in uploaded_files:
                ...
                # Save as image_1.jpg, image_2.jpg, ...
                image_path = person_dir / f"image_{next_num}{ext}"
                with open(image_path, "wb") as f:
                    f.write(uploaded_file.getbuffer())
                saved_paths.append(str(image_path))
                next_num += 1

            # Check if person already exists in session state
            existing_person = None
            for person in st.session_state.authorized_faces:
                if person["name"] == name:
                    existing_person = person
                    break

            if existing_person:
                # Append new image paths to existing person
                existing_person["image_paths"].extend(saved_paths)
                st.success(f"Added {len(saved_paths)} more image(s) to {name}.")
            else:
                # Create new entry
                ...
                st.success(f"{name} added with {len(saved_paths)} image(s).")
            st.rerun()
...
if st.session_state.authorized_faces:
    for i, person in enumerate(st.session_state.authorized_faces):
        ...
        col1.write(f"{person['name']} ({len(person['image_paths'])} images)")
        if col2.button("View", key=f"view_{i}"):
            with st.expander(f"Images for {person['name']}"):
                for idx, img_path in enumerate(person['image_paths']):
                    st.image(img_path, caption=f"Image {idx+1}", width=150)
        if col3.button("Remove", key=f"remove_auth_{i}"):
            # Optionally delete the folder? For now just remove from session.
